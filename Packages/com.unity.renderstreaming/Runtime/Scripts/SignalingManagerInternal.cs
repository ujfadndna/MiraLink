...
namespace Unity.RenderStreaming
{
        ...
        IRenderStreamingHandler, IRenderStreamingDelegate
    {
        ...
        void OnCreateConnection(ISignaling signaling, string connectionId, bool polite)
        {
            ...
            _startCoroutine(AddVideoTrackAndOfferIfNeeded(connectionId));
        }

        System.Collections.IEnumerator AddVideoTrackAndOfferIfNeeded(string connectionId)
        {
            yield return new UnityEngine.WaitForSeconds(0.5f);
            if (!_mapConnectionIdAndPeer.TryGetValue(connectionId, out var peerConn))
                yield break;

            if (HasVideoSender(peerConn))
            {
                peerConn.SendOffer();
                yield break;
            }

            var sender = UnityEngine.Object.FindObjectOfType<VideoStreamSender>();
            if (sender == null)
            {
                RenderStreaming.Logger.Log(LogType.Warning, $"No VideoStreamSender found for {connectionId}");
                yield break;
            }
            ...
            if (sender.Track == null)
            {
                ...
                if (op.Track == null)
                    yield return op;
                sender.SetTrack(op.Track);
            }

            if (sender.Track == null)
            {
                RenderStreaming.Logger.Log(LogType.Warning, $"VideoStreamSender track is null for {connectionId}");
                yield break;
            }

            if (!HasVideoSender(peerConn))
            {
                var init = new RTCRtpTransceiverInit { direction = RTCRtpTransceiverDirection.SendOnly };
                peerConn.peer.AddTransceiver(sender.Track, init);
                RenderStreaming.Logger.Log($"Added single video transceiver for {connectionId}");
            }

            peerConn.SendOffer();
        }

        bool HasVideoSender(PeerConnection peerConn)
        {
            return peerConn.peer.GetTransceivers().Any(t =>
                t.Sender != null &&
                t.Sender.Track != null &&
                t.Sender.Track.Kind == TrackKind.Video);
        }
