...
namespace Unity.RenderStreaming
{
    internal class PeerConnection : IDisposable
    {
        ...
        public PeerConnection(bool polite, RTCConfiguration config, float resendInterval, Func<IEnumerator, Coroutine> startCoroutine, Action<Coroutine> stopCoroutine)
        {
            ...
            RenderStreaming.Logger.Log($"ICE CONFIG input: {FormatIceConfig(config)}");
            _peer = new RTCPeerConnection(ref config);
            RenderStreaming.Logger.Log($"ICE CONFIG peer: {FormatIceConfig(_peer.GetConfiguration())}");
        ...
        private static string FormatIceConfig(RTCConfiguration config)
        {
            string policy = config.iceTransportPolicy.HasValue ? config.iceTransportPolicy.Value.ToString() : "null";
            if (config.iceServers == null || config.iceServers.Length == 0)
                return $"policy={policy}; servers=0";

            var parts = new List<string>();
            for (int i = 0; i < config.iceServers.Length; i++)
            {
                var server = config.iceServers[i];
                string urls = server.urls == null ? "" : string.Join(",", server.urls);
                string username = string.IsNullOrEmpty(server.username) ? "empty" : "set";
                int credentialLength = server.credential == null ? 0 : server.credential.Length;
                parts.Add($"#{i} urls=[{urls}] user={username} credType={server.credentialType} credLen={credentialLength}");
            }
            return $"policy={policy}; servers={config.iceServers.Length}; {string.Join(" | ", parts)}";
        }
        ...
        {
            if (TryAddIceCandidate(candidate))
                return true;

            if (!_ignoreOffer && ShouldBufferIceCandidate())
            {
                _pendingIceCandidates.Add(candidate);
                RenderStreaming.Logger.Log(LogType.Warning, $"{this} buffered ICE candidate until remote description is stable.");
                return true;
            }
        ...
        private bool TryAddIceCandidate(RTCIceCandidate candidate)
        {
            return candidate != null && _peer.AddIceCandidate(candidate);
        }

        private bool ShouldBufferIceCandidate()
        {
            return _processingSetDescription || _srdAnswerPending || _peer.SignalingState != RTCSignalingState.Stable;
        }
        ...
        private void FlushPendingIceCandidates()
        {
            ...
            var pending = _pendingIceCandidates.ToArray();
            _pendingIceCandidates.Clear();
            var accepted = 0;
            foreach (var candidate in pending)
            {
                ...
                RenderStreaming.Logger.Log(LogType.Warning, $"{this} buffered ICE candidate still can't accept on state.");
            }

            RenderStreaming.Logger.Log($"{this} flushed {accepted}/{pending.Length} buffered ICE candidates.");
        }
    }
}
